...
func (p *Provisioner) CapacitySummary(ctx context.Context) (host.CapacitySummary, error) {
	if p == nil || p.store == nil {
		return host.CapacitySummary{}, errors.New("provisioner is unavailable")
	}
	...
	if err != nil {
		return host.CapacitySummary{}, fmt.Errorf("read host memory status: %w", err)
	}
	memoryReserveBytes := hostMemoryReserveMiB * format.MiB
	...
	if err != nil {
		return host.CapacitySummary{}, err
	}
	...
	return host.CapacitySummary{
		...
		OS:       host.ReadOSInfo(),
		CPU:      cpuInfo,
		Instances: host.CapacityInstances{
			Total:   len(instances),
		...
		Capacity: []host.CapacityResource{
			{
				Resource:  "cpu",
				Unit:      "vcpu",
...
func (p *Provisioner) ensureHostMemoryReservationCapacity(ctx context.Context, excludeName string, requestedBytes int64) error {
	...
	if reservedBytes+requestedBytes > budgetBytes {
		return fmt.Errorf(
			...
			format.BinarySize(hostMemoryReserveMiB*format.MiB),
		)
	}
	return nil
}
...
func (p *Provisioner) capacityStorageDetails(ctx context.Context, path string) []host.CapacityDetail {
	details := storage.StorageDetails(ctx, path)
	result := make([]host.CapacityDetail, len(details))
	for i, d := range details {
		result[i] = host.CapacityDetail{Label: d.Label, Value: d.Value}
	}
	return result
}
