...
7. Writes `/root/.config/opencode/opencode.json` plus Pi config under `/root/.pi/agent/` to the per-VM host provider gateways when provider API keys are configured, or removes those managed defaults when all provider gateways are disabled
